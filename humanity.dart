...
@Command("q-card")
qcard(CommandEvent event) {
  event.reply(event.chooseAtRandom(questions));
}

@Command("card-me")
cardme(CommandEvent event) {
  int count = 1;
  
  if (!(event.hasOneArgument || event.hasNoArguments)) {
    event.usage();
    return;
  }
  ...
  if (event.argc == 1) {
    try {
      count = int.parse(event.args[0]);
    } catch (e) {
      event.reply("Invalid Number", prefixContent: "Humanity");
      return;
    }
  }
  
  for (var i = 1; i <= count; i++) {
    event.reply(event.chooseAtRandom(answers));
  }
}
